...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="result[@widget = 'news.rubric.list']/list/collection" mode="news.sidebar.list">
        <ul>
    ...
    <xsl:template match="result[@widget = 'news.rubric.list']/list/collection/item" mode="news.sidebar.list">
        <li>
            <a href="{@url}">
            ...
            <!-- Рекурсивный вызов -->
            <xsl:apply-templates select="document(concat('widget://news.rubric.list?parentRubric=', @guid))" mode="news.sidebar.list" />
        </li>
    </xsl:template>
