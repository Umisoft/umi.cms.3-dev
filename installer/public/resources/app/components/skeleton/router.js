...
//		});
		...
		UMI.ComponentRoute = Ember.Route.extend({
			model: function(params, transition){
				 ...
				 */
				return $.getJSON(componentResource).then(
					function(results){
						var nameModel;
						var modes;
						var hasTree;
		...
		UMI.ContentRoute = Ember.Route.extend({
			model: function(params){
				...
				var results = {};
				return $.getJSON(activeMode.get('resources')).then(
					function(data){
						...
						return self.store.find(self.modelFor('mode').get('contentType'), params.content).then(
							function(model){
