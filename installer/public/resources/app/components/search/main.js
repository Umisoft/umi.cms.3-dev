define(['App', 'text!./search.hbs'],
    function (UMI, searchTpl) {
        'use strict';
        ...
        UMI.SearchContentComponent = Ember.Component.extend({
            ...
            submit: function () {
                ...
                var result = [
                    {
                        'title': 'О нас',
                        'text': 'Сами разработчики UMI.CMS поставили «легкость» работы с системой в качестве одного из ' +
                            'основных концептуальных положений. Система изначально была написана на PHP5 (начинала разрабатываться она в 2004 г.,' +
                            ' когда его еще толком никто не использовал). Система создавалась от начала и до конца одной командой программистов. ' +
                            'Это можно отметить как большой плюс: действующие разработчики понимают систему полностью «изнутри».',
                        'slug': '2'
                    },
                    {
                        'title': 'Контакты',
                        'text': 'Телефон в Москве: +7 (495) 662-87-59 Телефон в Cанкт-Петербурге: +7 (812) 309-03-15' +
                            ' Бесплатный телефон для городов России: 8-800-5555-864 (для некоторых операторов +7-800-5555-864)',
                        'slug': '11'
                    }
                ];
